...
async function processClinicMissedAppointment() {
  logging('Info', '[clinicMissedAppointment] Job started');
  ...
  try {
    ...
    // const threeDaysAgoStr = '2025-08-27'; 

    logging('Info', `[clinicMissedAppointment] Checking appointments for date: ${threeDaysAgoStr}`);
    ...
    const appointmentsResult = await getFhirResource(
      ...
      'Appointment',
      null,
      `status=booked&date=${threeDaysAgoStr}`,
      null
    );

    if (!appointmentsResult.status) {
      logging('Info', `[clinicMissedAppointment] No booked appointments found for ${threeDaysAgoStr}`);
      return;
    }

    const appointments = appointmentsResult.response.entry || [];
    if (appointments.length === 0) {
      logging('Info', `[clinicMissedAppointment] No appointments found for ${threeDaysAgoStr}`);
      return;
    }
    ...
    for (const apptEntry of appointments) {
      ...
      if (!patientParticipant) {
        logging('Error', `[clinicMissedAppointment] Missing patient reference in Appointment/${appointment.id}`);
        continue;
      }
      ...
      if (!patientResult.status) {
        logging('Error', `[clinicMissedAppointment] Patient not found: ${patientId}`);
        continue;
      }
      ...
      if (!patientPhone) {
        logging('Error', `[clinicMissedAppointment] Missing phone number for patient ${patientId}`);
        continue;
      }
      ...
      const smsBody = smsTemplate('clinicMissedAppointment', languageToUse, {
        clientName: patientName,
        referralDate: threeDaysAgoStr,
        facilityName: facilityName
      });

      if (!smsBody) {
        logging('Error', `[clinicMissedAppointment] Failed to prepare SMS for patient ${patientId}`);
        continue;
      }
      ...
      if (!smsSent) {
        logging('Error', `[clinicMissedAppointment] Failed to send SMS to patient ${patientId}`);
        continue;
      }

      logging('Info', `[clinicMissedAppointment] SMS sent to patient ${patientId}`);
      ...
      if (!postResult.status) {
        logging('Error', `[clinicMissedAppointment] Failed to create Communication resource for patient ${patientId}`);
      } else {
        logging('Info', `[clinicMissedAppointment] Communication resource created for patient ${patientId}`);
      }
    }

    logging('Info', '[clinicMissedAppointment] Job finished');
  } catch (error) {
    logging('Error', `[clinicMissedAppointment] Exception: ${error.message || error}`);
  }
}

module.exports = { processClinicMissedAppointment };
