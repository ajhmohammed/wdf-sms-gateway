...
async function processClinicBeforeAppointmentDay() {
  logging('Info', '[clinicBeforeAppointmentDay] Job started');
  ...
  try {
    ...
    // const tomorrowStr = '2025-08-27';

    logging('Info', `[clinicBeforeAppointmentDay] Checking appointments for date: ${tomorrowStr}`);
    ...
    const appointmentsResult = await getFhirResource(
      ...
      'Appointment',
      null,
      `status=booked&date=${tomorrowStr}`,
      null
    );

    if (!appointmentsResult.status) {
      logging('Info', `[clinicBeforeAppointmentDay] No booked appointments found for ${tomorrowStr}`);
      return;
    }

    const appointments = appointmentsResult.response.entry || [];
    if (appointments.length === 0) {
      logging('Info', `[clinicBeforeAppointmentDay] No appointments found for ${tomorrowStr}`);
      return;
    }
    ...
    for (const apptEntry of appointments) {
      ...
      if (!patientParticipant) {
        logging('Error', `[clinicBeforeAppointmentDay] Missing patient reference in Appointment/${appointment.id}`);
        continue;
      }
      ...
      if (!patientResult.status) {
        logging('Error', `[clinicBeforeAppointmentDay] Patient not found: ${patientId}`);
        continue;
      }
      ...
      if (!patientPhone) {
        logging('Error', `[clinicBeforeAppointmentDay] Missing phone number for patient ${patientId}`);
        continue;
      }
      ...
      const smsBody = smsTemplate('clinicBeforeAppointmentDay', languageToUse, {
        clientName: patientName,
        referralDate: tomorrowStr,
        facilityName: facilityName
      });

      if (!smsBody) {
        logging('Error', `[clinicBeforeAppointmentDay] Failed to prepare SMS for patient ${patientId}`);
        continue;
      }
      ...
      if (!smsSent) {
        logging('Error', `[clinicBeforeAppointmentDay] Failed to send SMS to patient ${patientId}`);
        continue;
      }

      logging('Info', `[clinicBeforeAppointmentDay] SMS sent to patient ${patientId}`);
      ...
      if (!postResult.status) {
        logging('Error', `[clinicBeforeAppointmentDay] Failed to create Communication resource for patient ${patientId}`);
      } else {
        logging('Info', `[clinicBeforeAppointmentDay] Communication resource created for patient ${patientId}`);
      }
    }

    logging('Info', '[clinicBeforeAppointmentDay] Job finished');
  } catch (error) {
    logging('Error', `[clinicBeforeAppointmentDay] Exception: ${error.message || error}`);
  }
}

module.exports = { processClinicBeforeAppointmentDay };
